...
pub fn create_transaction_for_offer(offer: Offer) -> ExternResult<Hashed<Transaction>> {
    ...
    create_link(AgentPubKey::from(offer.spender_pub_key).into(), transaction_hash.clone(), ())?;
    create_link(AgentPubKey::from(offer.recipient_pub_key).into(), transaction_hash.clone(), ())?;
